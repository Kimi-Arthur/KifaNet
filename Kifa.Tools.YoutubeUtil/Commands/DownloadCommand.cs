...
public abstract class DownloadCommand : YoutubeCommand {
    ...
    KifaActionResult LinkExistingRepoFile(KifaFile repoFile, YouTubeVideo video,
        ...
        return KifaFile.LinkAll(repoFile, matchedTargetFiles);
    }
    ...
    public YouTubeUploader? EnsureUploaderInfo(YouTubeVideo video) {
        ...
        YouTubeUploader? uploader = null;
        if (video.AuthorId != null) {
            uploader = YouTubeUploader.Resolve(video.AuthorId, refresh: Refresh);
        }

        if (uploader == null && video.Author != null) {
            uploader = YouTubeUploader.Resolve(video.Author, refresh: Refresh);
        }
