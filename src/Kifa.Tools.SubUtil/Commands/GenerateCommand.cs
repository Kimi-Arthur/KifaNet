...
class GenerateCommand : KifaCommand {
    ...
    const string SubxGeneratorIndicator = "Generated with `subx generate`";
    ...
    KifaActionResult GenerateSubtitle(KifaFile file) {
        ...
        var scriptInfo = new AssScriptInfoSection {
            ...
            UpdateDetails = SubxGeneratorIndicator
        };
        ...
        var subList = selectedSubtitles.Status == KifaActionStatus.OK
        ...
        scriptInfo.OriginalScript =
            string.Join(", ", subtitleIds.Concat(biliChatList.Select(c => c.Id)));
    ...
    static List<(string Id, List<AssDialogue> Dialogs, List<AssStyle> Styles)> GetAssSubtitles(
        KifaFile rawFile) {
        var subtitles = new List<(string Id, List<AssDialogue> Dialogs, List<AssStyle> Styles)>();
        foreach (var file in rawFile.GetSubtitleFiles("*.ass")) {
            var document = AssDocument.Parse(file.OpenRead());
            var scriptInfo = document.Sections.OfType<AssScriptInfoSection>().FirstOrDefault();
            if (scriptInfo?.UpdateDetails == SubxGeneratorIndicator) {
                Logger.Debug($"Skipping subtitle {file} as it was generated by `subx generate`.");
                continue;
            }

            subtitles.Add((file.BaseName[(rawFile.BaseName.Length + 1)..],
