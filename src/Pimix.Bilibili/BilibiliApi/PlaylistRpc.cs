...
namespace Pimix.Bilibili.BilibiliApi {
    ...
    public class PlaylistRpc : JsonRpc<string, PlaylistResponse> {
        const string PlaylistUrlPattern = "https://api.bilibili.com/x/v3/fav/resource/list?media_id={id}&pn={page}&ps=20";
        ...
        public override PlaylistResponse Call(string playlistId) {
            ...
            while (result.Data.HasMore) {
                url = PlaylistUrlPattern.Format(new Dictionary<string, string> {{"id", playlistId}, {"page", (++page).ToString()}});
