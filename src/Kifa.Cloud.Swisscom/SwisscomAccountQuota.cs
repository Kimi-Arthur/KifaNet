...
public class SwisscomAccountQuota : DataModel, WithModelId<SwisscomAccountQuota> {
    ...
    static SwisscomAccountQuota? GetAccount(List<SwisscomAccountQuota> accounts, long length) {
        ...
        if (lookForAligned) {
            // There are no aligned accounts (+ empty accounts), maybe there are unaligned ones.
