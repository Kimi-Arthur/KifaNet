...
public class YouTubeVideoTests {
    ...
    public void FillWithYoutubeDlTest() {
        ...
        video.Fill();
        video.Id.Should().Be("_ox9gJZ8ENo");
        video.Title.Should().Be("SNH48 《呜吒》 (UZA) Rehearsal Footage");
        video.Author.Should().Be("SNH48s");
        video.AuthorId.Should().Be("@SNH48s");
        video.UploadDate!.ToJson().Should().Be("2014-08-22");
        video.Categories.Should().Contain("Entertainment");
        video.Tags.Should().Contain("SNH48");
        video.Duration.Should().Be(TimeSpan.FromSeconds(234));
    }

    [Fact]
    public void FillShortTest() {
        var video = new YouTubeVideo {
            Id = "WUj_TgtrTJE"
        };

        video.Fill();
        video.Id.Should().Be("WUj_TgtrTJE");
        video.Author.Should().Be("FC Bayern Munich");
        video.AuthorId.Should().Be("@fcbayern");
        video.Duration.Should().Be(TimeSpan.FromSeconds(29));
    }
