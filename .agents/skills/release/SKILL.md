...
2. **Categorize Change Type & Calculate Version Bump**:
   * Inspect diffs, recent commits, and added files to categorize the change:
...
3. **Handle Pending Changes (Two-Commit Workflow)**:
   * **If there are pending code changes**:
     * Split the release process into two separate, sequential commits:
       1. **First Commit (Functional Changes)**: Stage and commit the functional changes first. Follow the **Commit Skill** for this step, using standard conventional commit formats (e.g. `feat(filex): ...` or `fix(filex): ...`).
